...
class {:lucid_module} SlidingBloomFilter {                     
   ...
   ghost predicate inFilterWindow (key: uint32, ts : uint32)
      reads allObjs
      ...
      ensures unchanged(panes[(frontPane(ts)-1)%4], panes[(frontPane(ts)-2)%4])//warm panes
   {
      ...
      var p_idx := frontPane(ts);
      if (p_idx == 0) { panes[0].insert(key);  panes[1].clearCell(idx); }
      else if (p_idx == 1) { panes[1].insert(key);  panes[2].clearCell(idx); }
      else if (p_idx == 2) { panes[2].insert(key);  panes[3].clearCell(idx); }
      else if (p_idx == 3) { panes[0].clearCell(idx);  panes[3].insert(key); }
